...
//                int i  = scan.nextInt();
                ...
                for(int index = 0;index<= text.length-1;index++){
                    if (text[index] == null) {//만약에 첫번째 게시물의 정보가 비어있으면
                        text[index].content =  contentsc; text[index].writer = writersc;
...
//                }// else if end
            ...
            if (choose == 2) {//만약에 입력한 값이 2이면 '게시물 출력'구현
                for(int index = 0; index <= text.length-1 ; index++){
                    if(text[index].content!=null){//만약에 index 게시물이 존재하면
                        System.out.printf("작성자 : %s, 내용 : %s%n ", text[index], text[index]);
                    }
                }
