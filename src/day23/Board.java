...
// 클래스(도면지/설계도-코드저장) vs 객체(설계도기반의 메모리공간)
        //클래스 구성 멤버
        //1. 필드(멤버변수)
                //필드는 초기값이 없으면 정수:0; 실수:0.0; boolean:false;참조타입=null
                //주의할 점: 일반 지역변수는 초기값이 없으면 쓰레기갓이 들어간다.
...
class Board {
        ...
        private int pwd;
                ...
                //외부로쿠버 지정한 필드의 직접접근을 차단하기 위해

        //2. 생성자(객체초기화함수) : 객체생성시 new 뒤로 오는 함수
                //생성자는 클래스명과 이름이 동일해야 한다. 동일하지 않으면 함수로 취급.
                //-생성자는 매개변수(함수안으로 들어오는 값들 저장하는 변수)가 있지만
                //반환값(함수가 종료될때 반환하는 값)이 없다
                //오버로딩(@overloading) : 동일한 함수면, 동일한 생성자 명으로 선언은 불가능 하지만 매개변수의 타입, 개수, 순서에 따라 식별 가능한 함수 선언 방법
...
//3. 메소드(멤버함수)
        ...
        //주소값 대신 객체의 객체의 정보를 출력하기 위해 재정의한다.
        @Override
