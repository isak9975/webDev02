package day38.boardservice12mvc.model;
...
public boolean boardLogin(String id,int pwd){
        ...
        //id=?,pwd=?인 행을 찾아서 count(*)합을 표시한다 ==> id와 pwd 값이 일치한 행이 있을 경우 1을 반환 / 없을 경우 0;
        try {
                PreparedStatement ps = connection.prepareStatement(sql);
                //2. SQL 기재
                ps.setString(1,id); //첫번째 물음표에 id 넣기
                ps.setInt(2,pwd); //두번째 물음표에 pwd 넣기

                //3. 기재된 SQL 조작 = ps.executeQuery();
                ResultSet rs = ps.executeQuery(); //조회 결과를 ResultSet 형태(행)로 rs에 넣음

                if(rs.next()){ // 결과를 읽을 수 있다면 == sql 이 출력한 값이 있으면

                        int count = rs.getInt(1); //getInt(1) => 1열의 값 가져오기 = count(*)값 가져오기
